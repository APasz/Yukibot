...
class SteamUpdateConfig(BaseModel):
    ...
    def with_selected_branch(
        self,
        branch: SteamUpdateBranch | str,
        *,
        add_if_missing: bool = False,
    ) -> "SteamUpdateConfig":
        """Return this configuration with a selected, optionally newly registered Steam branch."""
        requested_branch = branch if isinstance(branch, SteamUpdateBranch) else SteamUpdateBranch(branch_id=branch)
        try:
            selected_branch = self.branch(requested_branch.branch_id)
        except ValueError:
            if not add_if_missing:
                raise
            selected_branch = requested_branch
            branches = (*self.branches, selected_branch)
        else:
            branches = self.branches
        return self.model_copy(update={"branches": branches, "selected_branch": selected_branch.branch_id})
