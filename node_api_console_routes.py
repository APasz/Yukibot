...
def register_console_routes(
    ...
    *,
    ...
    async def _list_console_actions(
        ...
        traffic_log.info("Node API console action list request: node=%s app=%s", service.node_name, app_name)
    ...
    async def _read_console_stdout_route(
        ...
        traffic_log.info("Node API console stdout request: node=%s app=%s", service.node_name, app_name)
    ...
    async def _console_stdout_stream(
        ...
        traffic_log.info("Node API console stdout stream request: node=%s app=%s", service.node_name, app_name)
        if max_lines < 1 or max_lines > 500:
