...
class ModWebUserAppearanceMixin(ModWebServiceSupport):
    ...
    def _user_tooltip_placement_javascript(tooltip_above_on_touch_device: bool) -> str:
        ...
        return (
            ...
            "const isTouchClient = () => {"
            "const hasCoarsePrimaryPointer = window.matchMedia?.('(pointer: coarse)').matches === true;"
            "const primaryPointerCanHover = window.matchMedia?.('(hover: hover)').matches === true;"
            "return hasCoarsePrimaryPointer && !primaryPointerCanHover;"
            "};"
