...
class ModWebStatusUtilitiesMixin(ModWebStatusFeatureSupport):
    def _render_user_utility_launcher(self, *, ui: ModWebUi, user: ModWebUser) -> None:
        ...
        open_unit_converter = self._build_unit_converter_panel(ui=ui)
    ...
    def _build_unit_converter_panel(self, *, ui: ModWebUi) -> Callable[[], None]:
        category_options: dict[str, str] = {
            category.value: category.display_name for category in unit_categories()
        }
        ...
        def _unit_options(category: UnitCategory) -> dict[str, str]:
            return {
                unit.code: f"{unit.display_name} · {unit.system.display_name}"
                for unit in units_for_category(category)
            }
        ...
        def _table_rows(
            *,
            ...
            converted_amounts = {
                conversion.target.code: conversion.converted_amount for conversion in conversions
            }
        ...
        def _selected_category(value_container: object) -> UnitCategory:
            ...
            if isinstance(raw_value, Mapping):
                candidates = [raw_value.get("value"), raw_value.get("label")]
                    ...
                    with ui.column().classes("w-full gap-4 mod-app-details-layout"):
                        ...
                        def _update_conversions(
                            ...
                            *,
                            ...
                            try:
                                resolved_category = (
                                    _selected_category(category_input) if category is None else category
                                )
                        ...
                        def _change_category(event: object | None = None) -> None:
                            ...
                            source_input.set_options(_unit_options(category), value=source.code)
                        ...
                        with ui.row().classes("w-full justify-end mod-app-details-actions"):
                            ui.button("Close", on_click=unit_converter_dialog.close).classes("mod-list-button secondary")
